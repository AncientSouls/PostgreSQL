...
const toValue = (x) => {
    ...
    if (x instanceof ExpSelect)
        return x.VALUE();
    if (_.isNumber(x) || _.isString(x) || _.isBoolean(x))
        return VALUES.DATA(x);
    return x;
};
class COMPARISONS {
    static EQ(a, b) {
...
class ExpSelect {
    ...
    WHERE(...where) {
        if (where.length === 1 && where[0] instanceof CONDITION) {
            ...
            this.where = new CONDITION(query_1.EExpConditionType.AND, where);
        }
        return this;
    }
    GROUP(...group) {
...
exports.ExpUnion = ExpUnion;
exports.UNION = (...selects) => {
    return new ExpUnion(query_1.EExpType.UNION, selects);
};
exports.UNIONALL = (...selects) => {
    return new ExpUnion(query_1.EExpType.UNIONALL, selects);
};
